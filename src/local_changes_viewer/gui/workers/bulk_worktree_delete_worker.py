...
class BulkWorktreeDeleteWorker(QRunnable):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Constructing the adapter is itself outside the per-item loop's
            # try/except below -- without this, a bad repo path or missing
            # git binary would raise straight out of run(), `finished` would
            # never fire, and BulkDeleteWorktreesDialog would be stuck
            # forever (list/buttons disabled, no way to close it). Reporting
            # every requested path as failed keeps the dialog's contract
            # ("finished always fires, failures land in `failed`") intact.
        ...
        for index, path in enumerate(self._paths, start=1):
            self.signals.progress.emit(index, total, str(path))
            # Deliberately force=False (see remove_worktree) -- a failure here
            # is reported per-item below rather than raised, so one dirty
            # worktree never aborts the rest of the batch.
            try:
